...
function externalWidgetUrl(input: string) {
    try {
        ...
        return match ? `https://iframe.x-downloader.com/${match[1]}/status/${match[2]}` : null;
    } catch { return null; }
}
export function VideoDownload() {
    const [backendReason, setBackendReason] = useState(''), [url, setUrl] = useState(''), [result, setResult] = useState<Result | null>(null), [error, setError] = useState(''), [loading, setLoading] = useState(false), [available, setAvailable] = useState<boolean | null>(null), [widgetUrl, setWidgetUrl] = useState<string | null>(null);
    ...
    async function inspect(event: FormEvent) {
        event.preventDefault(); setLoading(true); setError(''); setResult(null); setWidgetUrl(null);
        try {
    ...
    return <div className="space-y-6">
        ...
        <form onSubmit={inspect} className="flex min-w-0 flex-col gap-2 sm:flex-row"><input required type="url" maxLength={2048} value={url} onChange={(event) => { setUrl(event.target.value); setWidgetUrl(null); }} placeholder="https://x.com/user/status/123456789..." aria-label="Public X post URL" className="min-h-11 min-w-0 flex-1 rounded-xl border border-border bg-background px-4 text-sm" /><button type="submit" disabled={loading || available !== true} className="rounded-xl bg-foreground px-5 py-3 text-sm font-bold text-background disabled:opacity-50">{loading ? 'Inspecting...' : 'Find video'}</button></form>
        {error && <p role="alert" className="rounded-xl border border-rose-500/40 p-4 text-sm text-rose-500">{error}</p>}
        {(error || available === false) && externalWidgetUrl(url) && <section className="rounded-xl border border-border p-4"><p className="text-sm font-semibold">Alternative download</p><p className="mt-1 text-xs text-muted-foreground">X Downloader is an external service. Opening its widget sends the post link to x-downloader.com and uses its own download flow.</p><button type="button" onClick={() => setWidgetUrl(externalWidgetUrl(url))} className="mt-3 rounded-lg border border-border px-4 py-2 text-sm">Open X Downloader widget</button>{widgetUrl && <iframe title="X Downloader video download" src={widgetUrl} width="100%" height="64" className="mt-4 min-w-[250px] rounded-lg border border-border" referrerPolicy="no-referrer" />}</section>}
