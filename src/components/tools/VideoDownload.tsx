...
export function VideoDownload() {
    const [platform, setPlatform] = useState<'X' | 'Threads' | 'YouTube'>('X'), [backendReason, setBackendReason] = useState(''), [url, setUrl] = useState(''), [result, setResult] = useState<Result | null>(null), [error, setError] = useState(''), [loading, setLoading] = useState(false), [available, setAvailable] = useState<boolean | null>(null), [copied, setCopied] = useState(false);
    ...
    return <div className="space-y-6">
        <div role="tablist" aria-label="Video platform" className="flex gap-2">{(['X', 'Threads', 'YouTube'] as const).map((name) => <button key={name} type="button" role="tab" aria-selected={platform === name} onClick={() => { setPlatform(name); setUrl(''); setResult(null); setError(''); setCopied(false); }} className={`rounded-xl border px-5 py-2 text-sm font-semibold ${platform === name ? 'border-foreground bg-foreground text-background' : 'border-border'}`}>{name}</button>)}</div>
        <div className="rounded-2xl border border-border p-5"><p className="text-sm font-bold">Public {platform} videos</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Paste a direct public {platform} {platform === 'YouTube' ? 'video' : 'post'} URL. Private or login-only videos are unsupported. Download only material you own or are permitted to save. YouTube may not expose a direct MP4 for every video.</p><p className={`mt-3 text-xs ${available ? 'text-emerald-500' : 'text-amber-500'}`}>{available === null ? 'Checking video service...' : available ? `${platform} video service ready` : backendReason ? `Video service unavailable - ${backendReason}` : 'Video service unavailable.'}</p></div>
        <form onSubmit={inspect} className="flex min-w-0 flex-col gap-2 sm:flex-row"><input required type="url" maxLength={2048} value={url} onChange={(event) => { setUrl(event.target.value); setCopied(false); }} placeholder={platform === 'X' ? 'https://x.com/user/status/123456789...' : platform === 'Threads' ? 'https://www.threads.com/@user/post/ABC123' : 'https://www.youtube.com/watch?v=VIDEO_ID'} aria-label={`Public ${platform} video URL`} className="min-h-11 min-w-0 flex-1 rounded-xl border border-border bg-background px-4 text-sm" /><button type="submit" disabled={loading || available !== true} className="rounded-xl bg-foreground px-5 py-3 text-sm font-bold text-background disabled:opacity-50">{loading ? 'Inspecting...' : 'Find video'}</button></form>
        {error && <p role="alert" className="rounded-xl border border-rose-500/40 p-4 text-sm text-rose-500">{error}</p>}
        {platform === 'X' && (error || available === false) && validXPostUrl(url) && <section className="rounded-xl border border-border p-4"><p className="text-sm font-semibold">Alternative download</p><p className="mt-1 text-xs text-muted-foreground">The embedded X Downloader widget did not load reliably. Copy the post URL, then paste it into their website. Your link is not sent to them until you choose to paste it there.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void navigator.clipboard.writeText(validXPostUrl(url)!).then(() => setCopied(true)).catch(() => setError('Could not copy the link. Select it from the field above.'))} className="rounded-lg border border-border px-4 py-2 text-sm">{copied ? 'Copied' : 'Copy post URL'}</button><a href="https://x-downloader.com/en/" target="_blank" rel="noopener noreferrer" className="rounded-lg border border-border px-4 py-2 text-sm">Open X Downloader</a></div></section>}
