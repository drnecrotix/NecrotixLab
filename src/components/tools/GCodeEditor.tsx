...
export function GCodeEditor() {
    ...
    return <div className="space-y-5">
        <section className="grid overflow-hidden rounded-2xl border border-border lg:grid-cols-[1.05fr_.95fr]">
            <div className="border-b border-border lg:border-b-0 lg:border-r"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4"><div><p className="font-mono text-xs text-cyan-500">G-code program</p><p className="text-xs text-muted-foreground">Line {step || 1} of {analysis.lineCount}</p></div><div className="flex flex-wrap gap-2"><input ref={fileInput} className="hidden" type="file" accept=".nc,.txt,text/plain" onChange={(e) => void importFile(e.target.files?.[0])} /><Button onClick={() => fileInput.current?.click()}>Import .nc/.txt</Button><Button onClick={() => setCode(starter)}>Reset</Button><Button onClick={() => save(code, 'nc')}>Export .nc</Button><Button onClick={() => save(code, 'txt')}>Export .txt</Button></div></div>
                <div className="grid grid-cols-[3rem_1fr] bg-foreground/[.025]"><div ref={gutter} aria-hidden="true" className="h-[32rem] overflow-hidden border-r border-border py-4 text-right font-mono text-xs leading-6 text-muted-foreground/60">{code.split(/\r?\n/).map((_, i) => <div key={i} className={`pr-3 ${step === i + 1 ? 'bg-cyan-500/25 font-bold text-cyan-500' : ''}`}>{i + 1}</div>)}</div><textarea ref={textarea} aria-label="G-Code program" value={code} onScroll={(e) => { if (gutter.current) gutter.current.scrollTop = e.currentTarget.scrollTop; }} onChange={(e) => setCode(e.target.value)} spellCheck={false} className="h-[32rem] w-full resize-y bg-transparent p-4 font-mono text-xs leading-6 text-foreground outline-none focus:bg-cyan-500/[.025]" /></div>
            </div><div className="flex flex-col p-5"><div><p className="font-mono text-xs text-emerald-500">XY toolpath simulation</p><p className="text-xs text-muted-foreground">Amber: feed - cyan dashed: rapid - blue: executed - arcs sampled from I/J</p></div><div className="grid min-h-72 flex-1 place-items-center py-5"><svg role="img" aria-label="G-code XY simulation" viewBox={`${analysis.bounds.minX - width * .1} ${-analysis.bounds.maxY - height * .1} ${width * 1.2} ${height * 1.2}`} className="max-h-[27rem] w-full" preserveAspectRatio="xMidYMid meet">{segments.map(({ from, to, index }) => <path key={index} d={`M ${from.x} ${-from.y} L ${to.x} ${-to.y}`} fill="none" stroke="currentColor" strokeWidth={index === step ? 3 : 1.5} strokeDasharray={to.rapid ? '5 5' : undefined} className={index <= currentIndex ? 'text-blue-500' : to.rapid ? 'text-cyan-500' : 'text-amber-500'} vectorEffect="non-scaling-stroke" />)}<circle cx={current.x} cy={-current.y} r={Math.max(width, height) / 70} fill="currentColor" className="text-emerald-500" /></svg></div><div className="grid grid-cols-3 gap-2 border-t border-border py-3 text-center text-xs"><span>{analysis.motionCount} moves</span><span>{analysis.distance.toFixed(1)} units XY</span><span>{analysis.issues.length} review items</span></div>
