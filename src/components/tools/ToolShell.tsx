...
export function ToolShell({ eyebrow = 'Binary tools', title, description, children }: { eyebrow?: string; title: string; description: string; children: React.ReactNode }) {
    return (
        <main className="min-h-screen bg-background px-5 pb-24 pt-28 text-foreground sm:px-8 lg:pt-36">
            <div className="mx-auto max-w-5xl">
                ...
                <header className="mt-8 grid gap-5 border-b border-border/80 pb-8 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
                    ...
                    <div><p className="max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">{description}</p><p className="mt-4 inline-flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.16em] text-emerald-500"><LockKeyhole className="size-3.5" /> Local browser processing</p></div>
                </header>
