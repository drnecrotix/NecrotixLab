  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] select-none" aria-label={`${title} image viewer`}>
      ...
      >
        ...
        >
          <Image key={activeImage} src={activeImage} alt={activeIndex === 0 ? alt : `${alt} - ${activeIndex + 1}`} fill sizes="(max-width: 1180px) 100vw, 1180px" quality={86} priority={activeIndex === 0} unoptimized={shouldBypassImageOptimizer(activeImage)} draggable={false} onLoad={() => setImageLoaded(true)} className={cn('pointer-events-none object-contain transition-opacity duration-500 [-webkit-user-drag:none]', imageLoaded ? 'opacity-100' : 'opacity-0')} />
        </div>
      ...
      {safeImages.length > 1 && (
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Image series">
          {safeImages.map((image, index) => (
            ...
            >
              {image.startsWith('/') ? <Image retryable={false} src={image} alt="" fill sizes="112px" quality={60} loading="lazy" unoptimized={shouldBypassImageOptimizer(image)} draggable={false} className="pointer-events-none object-cover [-webkit-user-drag:none]" /> : <img src={image} alt="" loading="lazy" decoding="async" draggable={false} className="pointer-events-none absolute inset-0 h-full w-full object-cover [-webkit-user-drag:none]" />}
            </button>
          ))}
        </div>
      )}
