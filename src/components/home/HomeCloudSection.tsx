                ...
                <div className="min-w-0 overflow-hidden border border-foreground/10 bg-background/80">
                    ...
                    <div className="relative aspect-[5/4] min-h-0 w-full sm:aspect-[3/2]">
                        <svg viewBox="0 0 600 400" preserveAspectRatio="xMidYMid meet" className="block h-full w-full" role="img" aria-label="Animated diagram connecting files, access, sharing and synchronization to Kreatrics Cloud">
                            ...
                            <motion.circle cx="92" cy="82" r="4" fill="#38bdf8" initial={{ cx: 92, cy: 82, r: 4, opacity: 0 }} animate={moving ? { cx: [92, 300, 508, 300, 92], cy: [82, 200, 318, 200, 82], opacity: [0, 1, 1, 1, 0] } : { cx: 300, cy: 200, opacity: 1 }} transition={moving ? { duration: 7, repeat: Infinity, ease: 'linear' } : { duration: 0 }} />
                            <motion.circle cx="300" cy="200" r="60" fill="none" stroke="#38bdf8" strokeOpacity=".35" initial={{ r: 60, opacity: .35 }} animate={moving ? { r: [52, 70, 52], opacity: [.25, .55, .25] } : { r: 60, opacity: .35 }} transition={moving ? { duration: 3.2, repeat: Infinity, ease: 'easeInOut' } : { duration: 0 }} />
