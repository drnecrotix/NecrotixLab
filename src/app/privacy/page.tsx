        ...
        >
            ...
            <section>
                ...
                <ul className="mt-4 list-disc">
                    ...
                    <li><strong>Traffic analytics:</strong> country code, coarse city where available, broad device class, hourly visit/page-view aggregates, live traffic-session state and retained public-page activity. Page-activity rows can contain the public pathname, hashed traffic-session reference, country, city, device class and recent IP/network context such as IP version, ASN, ISP, organisation and network domain. Query strings and URL fragments are removed before page activity is stored. IP/network context is short-lived and is not kept for the full page-history retention period.</li>
            ...
            <section>
                ...
                <div className="mt-5 space-y-6">
                    ...
                    <div>
                        <h3>Operational error reporting</h3>
                        <p className="mt-2">The site records limited technical failures to diagnose broken pages, image loading and unsuccessful requests. Records contain the error category, a redacted route family, HTTP status when available, error code, timestamp and repeat count. Browser reports are unverified and are ignored when Do Not Track is enabled. Form values, cookies, query strings, raw IP addresses and full error messages are not stored in this log. Records remain available to authorised administrators for seven days and can be cleared manually. A/B testing has been retired and no new experiment events are collected.</p>
                    </div>
                    <div>
                        <h3>Short-retention traffic analytics</h3>
                        <p className="mt-2">NecrotixLab measures visits, current public pages, traffic trends, countries, coarse cities and broad device classes to understand which content and navigation visitors find useful and to identify operational or security patterns. Hosting/CDN country and city headers are preferred. When a public client IP can be read from trusted forwarding headers, the server may send it to <strong>ipwho.is</strong> to resolve country, coarse city and network-provider context such as ASN, ISP, organisation and network domain. Latitude, longitude and other precise coordinates are not stored. Recent IP/network context is available to authorised administrators for up to about 24 hours and is then cleared while non-IP page activity may remain for up to about 30 days. Authorised administrators can export the same access-controlled, retention-bounded activity fields for analysis; the export does not extend server retention. The public page-view tracker ignores tracking when the browser sends the Do Not Track signal. The legal basis relied on for this limited measurement is the legitimate interest in maintaining, securing and improving the site with bounded retention and data minimisation (Article 6(1)(f)).</p>
                    </div>
                    <div>
                        <h3>AI assistant</h3>
            ...
            <section>
                ...
                <p className="mt-3">Hosting or proxy infrastructure may provide country-level and coarse city request headers used by the first-party traffic analytics layer. The server may send a public client IP address to <strong>ipwho.is</strong> to resolve a country code, coarse city, ASN, ISP, organisation and network domain. NecrotixLab does not store latitude, longitude or other precise coordinates returned by a geolocation provider. Raw IP and associated network context remain only for the 24-hour IP-retention window and are not retained for the full page-activity or country/device aggregate retention period.</p>
            ...
            <section>
                ...
                <ul className="mt-4 list-disc">
                    ...
                    <li>Retained page-activity rows can remain for up to about 30 days and contain the public pathname, hashed traffic-session reference, country, coarse city and broad device class. Raw IP, ASN, ISP, organisation and network-domain values in those rows are cleared after about 24 hours, so older page history remains available without the IP/network context. Country/device aggregates are also removed after about 30 days. Automated cleanup is claimed at most once every six hours and runs on the next page-view request.</li>
