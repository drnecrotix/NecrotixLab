...
function RangeRows({ rows }: { rows: { id: string; name: string; min: number; max: number | null }[] }) {
    return <tbody>{rows.map((row) => <tr key={row.id} className="border-b border-white/10"><th className="py-4 pr-5 font-semibold text-white">{row.name}</th><td className="py-3 pr-4"><label className="block"><span className="mb-1 block text-xs text-white/65">EUR · one-off</span><input aria-label={`${row.name}: minimum one-off price in EUR`} name={`${row.id}-min`} type="number" min="0" max="50000" defaultValue={row.min} className={inputClass} /></label></td><td className="py-3"><label className="block"><span className="mb-1 block text-xs text-white/65">EUR · one-off</span><input aria-label={`${row.name}: maximum one-off price in EUR`} name={`${row.id}-max`} type="number" min="0" max="50000" defaultValue={row.max ?? ''} placeholder={row.max === null ? 'No maximum' : undefined} className={inputClass} /></label></td></tr>)}</tbody>;
}

function FixedRows({ rows }: { rows: { id: string; name: string; price: number }[] }) {
    return <tbody>{rows.map((row) => <tr key={row.id} className="border-b border-white/10"><th className="py-4 pr-5 font-semibold text-white">{row.name}</th><td className="py-3"><label className="block"><span className="mb-1 block text-xs text-white/65">EUR / month</span><input aria-label={`${row.name}: monthly price in EUR`} name={`${row.id}-price`} type="number" min="0" max="50000" defaultValue={row.price} className={inputClass} /></label></td></tr>)}</tbody>;
}
...
function PricingSection({ title, detail, children, fixed = false }: { title: string; detail: string; children: React.ReactNode; fixed?: boolean }) {
    return <section className="mt-10"><div className="flex flex-wrap items-center gap-3"><h2 className="text-xl font-black text-white">{title}</h2><span className="rounded border border-white/20 px-2 py-1 text-xs font-semibold text-white/80">{fixed ? 'Monthly subscription' : 'One-off payment'}</span></div><p className="mt-2 text-xs leading-5 text-white/45">{detail}</p><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="border-b border-white/10 text-xs text-white/45"><tr><th className="py-3">Service</th>{fixed ? <th className="py-3">Price (EUR / month)</th> : <><th className="py-3">Minimum (EUR, one-off)</th><th className="py-3">Maximum (EUR, one-off)</th></>}</tr></thead>{children}</table></div></section>;
}
