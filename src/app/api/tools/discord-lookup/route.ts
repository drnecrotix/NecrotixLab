...
export async function GET(request: NextRequest) {
    ...
    try {
        if (kind === 'invite') {
            ...
            return NextResponse.json({ kind, code: text(data.code), url: `https://discord.gg/${encodeURIComponent(query)}`, server: { id: text(guild.id), name: text(guild.name), description: text(guild.description), iconUrl: cdnAvatar(text(guild.id), text(guild.icon), 'icons'), bannerUrl: cdnAvatar(text(guild.id), text(guild.banner), 'banners'), vanityUrl: text(guild.vanity_url_code) ? `https://discord.gg/${encodeURIComponent(text(guild.vanity_url_code))}` : null, verificationLevel: number(guild.verification_level), features: Array.isArray(guild.features) ? guild.features.filter((v): v is string => typeof v === 'string').slice(0, 30) : [], members: number(data.approximate_member_count), online: number(data.approximate_presence_count), created: text(guild.id) ? snowflakeDate(text(guild.id)) : null }, channel: text(channel.name), expires: text(data.expires_at), note: 'Counts are approximate. Only information exposed by this public invite is shown.' }, { headers: { 'Cache-Control': 'no-store' } });
        }
        if (kind === 'server') {
            const data = await discord(`/guilds/${query}/widget.json`);
            return NextResponse.json({ kind, server: { id: query, name: text(data.name), iconUrl: `https://cdn.discordapp.com/guilds/${query}/widget.png?style=shield`, online: number(data.presence_count), created: snowflakeDate(query), invite: text(data.instant_invite), channels: Array.isArray(data.channels) ? data.channels.length : 0 }, note: 'The server must enable its public widget. This endpoint does not reveal private members or channels.' }, { headers: { 'Cache-Control': 'no-store' } });
        }
        ...
        return NextResponse.json({ kind, user: { id: query, username: text(data.username), displayName: text(data.global_name), avatarUrl: cdnAvatar(query, text(data.avatar), 'avatars') || `https://cdn.discordapp.com/embed/avatars/${Number(BigInt(query) >> 22n) % 6}.png`, bannerUrl: cdnAvatar(query, text(data.banner), 'banners'), accentColor: number(data.accent_color), bot: data.bot === true, system: data.system === true, created: snowflakeDate(query) }, note: 'Only profile fields available to the configured bot are shown.', profileUrl: `https://discord.com/users/${query}` }, { headers: { 'Cache-Control': 'no-store' } });
