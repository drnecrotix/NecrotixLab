...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (!process.env.DISCORD_BOT_TOKEN) return NextResponse.json({ kind, configurationRequired: true, user: { id: query, username: '', displayName: '', bot: null, created: snowflakeDate(query) }, note: 'Profile data and avatar cannot be retrieved from a User ID until the site owner configures DISCORD_BOT_TOKEN on the server. The decoded creation date does not verify this account.', profileUrl: `https://discord.com/users/${query}` }, { headers: { 'Cache-Control': 'no-store' } });
