...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (!process.env.DISCORD_BOT_TOKEN) return NextResponse.json({ error: 'User lookup requires a configured Discord bot token. Public Discord API does not expose arbitrary user profiles anonymously.' }, { status: 503 });
