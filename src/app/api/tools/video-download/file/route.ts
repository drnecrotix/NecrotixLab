...
import { streamVideo, videoBackendAvailable } from '@/modules/video-download/runner';
export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';
const downloads = new Map<string, { count: number; expires: number }>();
export async function GET(request: NextRequest) {
    try {
        if (!process.env.AUTH_SECRET || !await videoBackendAvailable()) return NextResponse.json({ error: 'Video backend unavailable.' }, { status: 503 });
        ...
        const stream = streamVideo(data.url, data.formatId);
